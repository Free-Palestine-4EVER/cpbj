      ...
      <div className="shell">
        ...
        <RevealGroup className="mt-16 grid gap-px overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--line)] md:grid-cols-3">
          {imperatives.cards.map((c) => (
            <RevealItem key={c.no} className="group relative bg-surface p-8 md:p-10">
              <div className="flex items-center justify-between">
                <span className="numeral text-[2.6rem] text-ghost transition-colors duration-500 group-hover:text-ember">
                  {c.no}
                </span>
                <span className="h-2 w-2 rounded-full bg-ember/70 transition-transform duration-500 group-hover:scale-150" />
              </div>
